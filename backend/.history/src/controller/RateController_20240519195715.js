...
const createRate = async (req, res) => {
    const { userId, podcastId, rating } = req.body
...
const updateRate = async (req, res) => {
    ...
    const { userId, rates, ...data} = req.body
    if (!RateId || !Array.isArray(rates)) {
        return res.status(404).json({
            status: "ERR",
    ...
    if (!rates.every(rate => mongoose.Types.ObjectId.isValid(rate))) {
        throw new Error("Invalid user ID format in Rates");
    }
