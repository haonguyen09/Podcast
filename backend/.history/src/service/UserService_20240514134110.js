...
const getAllUser = (limit = 4, page = 0, filter) => {
    return new Promise((resolve, reject) => {
        try {
            const totalUser = User.countDocuments()
            const condition = {}
            if (filter) {
                condition[filter[0]] = condition[{ 'reg': filter[0], 'options': 'i' }]
            }
            const response = await User.find(condition).limit().skip(limit * page)
            if (response) {
                resolve({
                    status: "OK",
