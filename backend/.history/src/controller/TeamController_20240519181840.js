...
const createTeam = async (req, res) => {
    const { name, userId, members, ...data } = req.body
...
const updateTeam = async (req, res) => {
    ...
    const { userId, members, name} = req.body
    if (!teamId || !Array(members)) {
        return res.status(404).json({
            status: "ERR",
    ...
    try {
        ...
        const response = await TeamService.updateTeam(commentId, updateData)
        if (response) {
            return res.status(200).json(response)
        }
    } catch (error) {
