...
const deleteTopic = (id) => {
    return new Promise( async (resolve, reject) => {
        ...
        try {
            ...
            if (response) {
                ...
                response.podcasts.forEach( async podcast => {
                    await Podcast.findByIdAndDelete(podcast)
                });
        ...
        try {
            const files = await fs.readdir(uploadsDir); // Read directory contents
            const filesToDelete = new Set([response.icon, response.image]); // Files to delete
