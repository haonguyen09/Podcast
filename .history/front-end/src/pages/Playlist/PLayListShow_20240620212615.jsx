    ...
    return (
        ...
        <div className='playlistShow-list d-grid'>
            <div className='playlistShow-item d-flex align-items-center'>
                <img src={`${REACT_APP_BACKEND}/uploads/${data[currentAudioIndex].image}`} alt='img' />
                <div className='playlistShow-info'>
                        <PodcastItemComponent audioUrl={`${REACT_APP_BACKEND}/uploads/${data[currentAudioIndex].audio}`} classWave='playlistShow-width' data={ data[currentAudioIndex] } />
                                ...
                                data.map((item, index) => (
                                    <div className='playlistShow-itemPodcast d-flex align-items-center justify-content-between' style={{cursor: "pointer"}} key={index} onClick={() => handleAudioItemClick(index)}>
                                        <div className='playlistShow-itemPodcast-info d-flex align-items-center'>
                                            <img src={`${REACT_APP_BACKEND}/uploads/${item.image}`} alt='img' />
                                            <h4>{item.title}</h4>
                                        </div>
            ...
            <audio ref={audioRef} src={`${REACT_APP_BACKEND}/uploads/${data[currentAudioIndex].audio}`} onEnded={handleAudioEnded} />
    </div>
    )
}
