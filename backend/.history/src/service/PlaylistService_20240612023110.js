    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (newPlaylist) {
            ...
            try {
                ...
                if (response) {
                    const suggestFollowId = await SuggestFollower.create({playlistId: response._id})
                    const followerId = await Follower.create({playlistId: response._id})
                    const followingId = await Following.create({playlistId: response._id})
                    await Playlist.findByIdAndUpdate(response._id, {
...
const updatePlaylist = (id, data, files) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const existingPlaylistByEmail = await Playlist.findOne({ email: data.email, _id: { $ne: playlistId } });

            if (existingPlaylistByEmail) {
                return reject({
                    status: "ERR",
