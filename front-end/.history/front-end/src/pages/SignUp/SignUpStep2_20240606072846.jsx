...
const SignUpStep2 = (props) => {
    ...
    const [selectedTopics, setSelectedTopics] = useState(() => {
        ...
        const savedTopics = getObjectFromLocalStorage('signUpData')
        return savedTopics ? savedTopics.topics : [];
    })
    ...
    const handleToggleChange = (id, status) => {
        setSelectedTopics(prevSelectedTopics => {
            let updatedTopics;
            if (status) {
                updatedTopics = [...prevSelectedTopics, id]
            } else {
                updatedTopics = prevSelectedTopics.filter(topicId => topicId !== id)
            }
            // Update localStorage
            // localStorage.setItem('selectedTopics', JSON.stringify(updatedTopics))h
                    ...
                    loading ? (
                        ...
                        listTopics.map((topic, index) => (
                            <div className='signUp-s2-item d-flex justify-content-between align-items-center' key={index}>
                                ...
                                <Toggle id={topic._id} onToggle={handleToggleChange} initialStatus={selectedTopics.includes(topic.id)}/>
                            </div>
                        ))
                    ) : (
                        <p>No topics found</p>
