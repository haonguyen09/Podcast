...
const PodcastItemComponent = (props) => {
    ...
    useEffect(() => {
        ...
        console.log("wavesurferRef.current.destroy", wavesurferRef.current.destroy)

        return () => {
            if (wavesurferRef.current) {
                wavesurferRef.current.unAll();
                if (wavesurferRef.current.destroy) {
                    try {
                        // Delay the destruction slightly to ensure safe cleanup
                        setTimeout(() => {
                            wavesurferRef.current.destroy();
                        }, 480); // Adjust the delay as needed
                    } catch (error) {
