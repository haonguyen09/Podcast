        ...
        if (newPodcast) {
            ...
            try {
                ...
                if (response) {
                    ...
                    // if (updateTopic) {
                    //     updateTopic.totalPodcast += 1;
                    //     if (!updateTopic.podcasts.includes(response._id)) {
                    //         updateTopic.podcasts.push(response._id);
                    //     }
    ...
    return new Promise( async (resolve, reject) => {
        ...
        try {
            ...
            if (response) {
                ...
                await removeFile(response.image, response.audio)
                //Handle remove file such as icon, image
                const uploadsDir = path.join(__dirname, '../uploads/');

                try {
                    const files = await fs.readdir(uploadsDir); 
                    const filesToDelete = new Set([response.image, response.audio]); 
