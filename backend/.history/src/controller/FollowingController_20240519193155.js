...
const createFollowing = async (req, res) => {
    ...
    try {
        ...
        if (!userId || !Array.isArray(followings)) {
...
const updateFollowing = async (req, res) => {
    ...
    if (!FollowingId || !Array(Followings)) {
        return res.status(404).json({
            status: "ERR",
    ...
    try {
        ...
        const response = await FollowingService.updateTeam(commentId, updateData)
        if (response) {
            return res.status(200).json(response)
        }
    } catch (error) {
